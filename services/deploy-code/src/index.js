...
async function handle(req, res) {
  ...
  if (url.pathname === '/upload-zip' && req.method === 'POST') {
  ...
  if (url.pathname === '/containers/logs' && req.method === 'GET') {
  ...
  if (url.pathname === '/containers/logs' && req.method === 'POST') {
    const body = await parseJsonBody(req).catch(() => ({}));
    return responseJson(res, 200, await readTargetLogs(body, cfg));
  }

  if (url.pathname === '/containers/inspect' && req.method === 'GET') {
  ...
  if (url.pathname === '/containers/inspect' && req.method === 'POST') {
    const body = await parseJsonBody(req).catch(() => ({}));
    return responseJson(res, 200, await inspectContainers(body, cfg));
  }

  if (url.pathname === '/containers/action' && req.method === 'POST') {
    const body = await parseJsonBody(req).catch(() => ({}));
    const result = await controlTargets(body.action, body, cfg);
    return responseJson(res, 200, { ok: true, result, status: await statusPayload() });
  }

  const actionMatch = url.pathname.match(/^\/containers\/(start|stop|restart|rebuild|up)$/);
